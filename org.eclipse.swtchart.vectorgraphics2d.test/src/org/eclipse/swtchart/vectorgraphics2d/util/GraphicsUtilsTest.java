...
public class GraphicsUtilsTest {
	...
	public static void setUpClass() {
		...
		{
			blackGrayscaleImage = new BufferedImage(TEST_IMAGE_WIDTH, TEST_IMAGE_HEIGHT, BufferedImage.TYPE_BYTE_GRAY);
			Graphics g = blackGrayscaleImage.getGraphics();
			g.setColor(Color.BLACK);
			g.fillRect(0, 0, TEST_IMAGE_WIDTH, TEST_IMAGE_HEIGHT);
		}
		{
			whiteGrayscaleImage = new BufferedImage(TEST_IMAGE_WIDTH, TEST_IMAGE_HEIGHT, BufferedImage.TYPE_BYTE_GRAY);
	...
	public void toBufferedImageCanConvertImageWithAlphaChannel() {
		...
		Image image = new BufferedImage(TEST_IMAGE_WIDTH, TEST_IMAGE_HEIGHT, BufferedImage.TYPE_INT_ARGB);
	...
	public void toBufferedImageCanConvertImageWithoutAlphaChannel() {
		...
		Image image = new BufferedImage(TEST_IMAGE_WIDTH, TEST_IMAGE_HEIGHT, BufferedImage.TYPE_INT_RGB);
		BufferedImage result = GraphicsUtils.toBufferedImage(image);
		assertBufferedImageEquals(image, result);
	}
	...
	public void toBufferedImageCanConvertFilteredImage() {
		...
		Image image = Toolkit.getDefaultToolkit().createImage(new FilteredImageSource(new BufferedImage(TEST_IMAGE_WIDTH, TEST_IMAGE_HEIGHT, BufferedImage.TYPE_INT_RGB).getSource(), new RGBImageFilter() {
	...
	public void toBufferedImageCanConvertRenderedImage() {
		...
		final BufferedImage bimage = new BufferedImage(TEST_IMAGE_WIDTH, TEST_IMAGE_HEIGHT, BufferedImage.TYPE_INT_RGB);
		RenderedImage image = new RenderedImage() {

			@Override
			public Vector<RenderedImage> getSources() {

				return bimage.getSources();
			}
			...
			public int getMinX() {
				...
				return bimage.getMinX();
			}

			@Override
			public int getMinY() {

				return bimage.getMinY();
			}

			@Override
			public int getNumXTiles() {

				return bimage.getNumXTiles();
			}

			@Override
			public int getNumYTiles() {

				return bimage.getNumYTiles();
			}

			@Override
			public int getMinTileX() {

				return bimage.getMinTileX();
			}

			@Override
			public int getMinTileY() {

				return bimage.getMinTileY();
			}

			@Override
			public int getTileWidth() {

				return bimage.getTileWidth();
			}

			@Override
			public int getTileHeight() {

				return bimage.getTileHeight();
			}

			@Override
			public int getTileGridXOffset() {

				return bimage.getTileGridXOffset();
			}

			@Override
			public int getTileGridYOffset() {

				return bimage.getTileGridYOffset();
			}

			@Override
			public Raster getTile(int tileX, int tileY) {

				return bimage.getTile(tileX, tileY);
			}
			...
			public Raster getData(Rectangle rect) {
				...
				return bimage.getData(rect);
			}

			@Override
			public WritableRaster copyData(WritableRaster raster) {

				return bimage.copyData(raster);
			}
		};
		BufferedImage result = GraphicsUtils.toBufferedImage(image);
		assertBufferedImageEquals(image, result);
	...
	public void usesAlphaReturnsFalseForImageWithoutTransparentPixels() {
		...
		BufferedImage image = new BufferedImage(TEST_IMAGE_WIDTH, TEST_IMAGE_HEIGHT, BufferedImage.TYPE_INT_ARGB);
	...
	public void getAlphaImageReturnsWhiteImageForInputWithoutAlphaChannel() {
		...
		BufferedImage image = new BufferedImage(TEST_IMAGE_WIDTH, TEST_IMAGE_HEIGHT, BufferedImage.TYPE_INT_RGB);
	...
	public void getAlphaImageReturnsBlackImageForTransparentInputWithBitmaskAlpha() {
		...
		ColorModel colorModel = new ComponentColorModel(colorSpace, true, false, Transparency.BITMASK, DataBuffer.TYPE_BYTE);
		WritableRaster raster = Raster.createInterleavedRaster(DataBuffer.TYPE_BYTE, TEST_IMAGE_WIDTH, TEST_IMAGE_WIDTH, 4, null);
